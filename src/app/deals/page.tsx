    ...
    <div style={{ background:'white', borderRadius:12, border:s.border, overflow:'hidden', display:'flex', flexDirection:'column' }}>
      <div style={{ background:s.bar, padding:'10px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        {deal.timerVal > 0
          ...
          : <span style={{ fontFamily:'Syne', fontWeight:600, fontSize:12, color:s.barText }}>{(deal as any).urgency}</span>
        }
      </div>
      <div style={{ padding:20, flex:1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <span style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 18, color: 'var(--nightsurf)' }}>{deal.depart}</span>
          <span style={{ fontFamily: 'Syne', fontSize: 12, color: 'var(--lagoon-muted)' }}>→</span>
          <span style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 18, color: 'var(--nightsurf)' }}>{deal.arrive}</span>
          <span style={{ fontFamily: 'Syne', fontSize: 11, color: 'var(--lagoon-muted)', marginLeft: 4 }}>· {deal.duration}</span>
        </div>
    ...
    <div>
      ...
      <div style={{ background:'var(--nightsurf)', padding: isMobile ? '32px 20px 40px' : '48px 40px 56px' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <h1 style={{ fontFamily:'Syne', fontWeight:800, fontSize: isMobile ? 28 : 40, color:'white', margin:'16px 0 8px', lineHeight:1.1 }}>Best flight deals to Siargao right now</h1>
          <p style={{ fontFamily:'Syne', fontWeight:400, fontSize: isMobile ? 14 : 16, color:'rgba(255,255,255,0.5)' }}>Updated daily. We track every Sunlight Air fare so you don't have to.</p>
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, auto)', gap: isMobile ? 10 : 12, marginTop:24 }}>
            {stats.map((s,i) => (
              <div key={i} style={{ background:s.color, borderRadius:10, padding: isMobile ? '14px 18px' : '16px 24px' }}>
                <p style={{ fontFamily:'Syne', fontWeight:800, fontSize: isMobile ? 20 : 24, color:s.textColor }}>{s.value}</p>
      ...
      {/* Content */}
      <div style={{ background:'var(--seafoam)', padding: isMobile ? '24px 16px 40px' : '32px 40px 48px' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          {/* Filter tabs */}
          <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
            {TABS.map(t => (
              <button key={t} onClick={() => setFilter(t)} style={{
                ...
                fontFamily:'Syne', fontWeight: filter===t ? 600 : 400, fontSize: isMobile ? 12 : 13,
                cursor:'pointer',
              }}>
                {TAB_LABELS[t]}
              </button>
